//#region node_modules/.nitro/vite/services/ssr/assets/parse-specs-C1H6yj3R.js
var GPU_VRAM = [
	{
		re: /rtx\s*5090/i,
		name: "RTX 5090",
...
function parseSpecsHeuristic(text) {
	...
	let gpuCount = 1;
	const multiMatch = raw.match(/\b([2-8])\s*x\s*(?:rtx|rx|nvidia|geforce|a100|h100|h200|l40|gpu|\d{4})/i) || raw.match(/\b(?:dual|2x)\s+(?:rtx|geforce|gpu|\d{4})/i) || raw.match(/\b(?:quad|4x)\s+(?:rtx|geforce|gpu|\d{4})/i) || raw.match(/\b(?:octa|8x)\s+(?:rtx|geforce|gpu|\d{4})/i);
	if (multiMatch) {
		if (multiMatch[1]) gpuCount = parseInt(multiMatch[1], 10);
		else if (/dual|2x/i.test(multiMatch[0])) gpuCount = 2;
		else if (/quad|4x/i.test(multiMatch[0])) gpuCount = 4;
		else if (/octa|8x/i.test(multiMatch[0])) gpuCount = 8;
	} else {
		const nvidiaSmiGpuMatches = raw.match(/\|\s*([0-7])\s+NVIDIA\s+/gi) || raw.match(/GPU\s+([0-7]):/gi);
		if (nvidiaSmiGpuMatches && nvidiaSmiGpuMatches.length > 1) gpuCount = Math.min(8, nvidiaSmiGpuMatches.length);
		else {
			const rtxOccurrences = (raw.match(/rtx\s*\d{4}/gi) ?? []).length;
			if (rtxOccurrences > 1) gpuCount = Math.min(8, rtxOccurrences);
		}
	}
...
//#endregion
export { specsFromUnknown as n, parseSpecsHeuristic as t };
